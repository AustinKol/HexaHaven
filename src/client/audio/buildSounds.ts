...
/** Gain before master volume — win fanfare one-shot. */
const BASE_VICTORY_FANFARE_VOLUME = 0.65;
...
/** Plays the game-winning fanfare once when a winner is announced. */
export function playVictoryFanfareSound(): void {
  if (!loadSettings().sfxEnabled) {
    return;
  }
  const audio = new Audio('/audio/victory-fanfare.mp3');
  audio.volume = scaledGameSfxVolume(BASE_VICTORY_FANFARE_VOLUME);
  void audio.play().catch(() => {
    // Autoplay policy or missing file — ignore
  });
}
